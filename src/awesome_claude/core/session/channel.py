...
class SessionChannel:
    ...
    def attach(self, session_id: str, *, ephemeral: bool = False) -> dict[str, Any]:
        """订阅到指定会话并返回回放状态。

        Args:
            session_id: 会话标识。
            ephemeral: 会话不存在时是否创建为临时会话。

        Returns:
            会话回放状态字典。
        """
        self._registry.get_or_create(session_id, ephemeral=ephemeral)
    ...
    async def detach(self) -> None:
        ...
        await self._registry.detach(self._sink)
    ...
    async def broadcast(
        ...
        if self._session_id is not None:
            await self.broadcast_to(
                self._session_id, method, params, exclude_self=exclude_self
            )
        else:
    ...
    async def broadcast_to(
        ...
        *,
        ...
        """向指定会话扇出通知，不依赖本连接的当前订阅状态。

        供在途 Run 使用：即使发起连接已断开，仍可把输出送达会话内
        其余订阅者。
    ...
    def record_turn(
        ...
        *,
        ...
        session = self._registry.get(target) if target else None
        if session is None:
            return
